...
function FInput({ label, type = "text", placeholder, value, onChange, max, min }: {
  label: string; type?: string; placeholder?: string; value: string;
  onChange: (v: string) => void; max?: string; min?: string;
}) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <label style={{ color: "rgba(255,255,255,0.6)", fontSize: "0.8rem", fontWeight: 500 }}>{label}</label>
      <input type={type} placeholder={placeholder} value={value} max={max} min={min}
...
function FNumber({ label, value, onChange, min = 0, max = 20 }: {
...
function Btn({ children, onClick, loading, disabled }: {
  ...
  return (
    <button type="button" onClick={onClick} disabled={loading || disabled}
      style={{ width: "100%", height: 54, borderRadius: 999, background: "white", color: "#005F2D", fontWeight: 700, fontSize: "1rem", border: "none", cursor: (loading || disabled) ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: (loading || disabled) ? 0.6 : 1 }}>
...
function ProgressBar({ step }: { step: Step }) {
  ...
  const pct = Math.round(((step as number - 1) / 6) * 100);
  const label = `Étape ${(step as number) - 1} sur 6`;
  return (
...
export default function RegisterPage() {
  ...
  async function save(s: 2 | 3 | 4 | 5 | 6 | 7) {
    setLoading(true); setError("");
    const bodies: Record<number, object> = {
      ...
      6: { step: 6, email, nombre_enfants: nbEnfants, personnes_a_charge: personnesCharge, ayants_droit: ayantsDroit },
      7: { step: 7, email, telephone: phone },
    };
    ...
    if (s === 7) { setIban(data.iban || ""); setStep("done"); }
  ...
  function updateAyantDroit(idx: number, field: keyof AyantDroit, val: string) {
    const next = [...ayantsDroit];
    next[idx] = { ...next[idx], [field]: val };
    setAyantsDroit(next);
  }

  function removeAyantDroit(idx: number) {
    setAyantsDroit(ayantsDroit.filter((_, i) => i !== idx));
  }
  ...
  if (step === 2) return (
    <div style={wrap}>
      ...
      <div style={{ flex: 1, paddingTop: 20, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <FSelect label="Source de revenus principale" value={source} onChange={setSource} options={SOURCES} />
          ...
          {error && <p style={{ color: "#FF6B6B", fontSize: "0.85rem" }}>{error}</p>}
          <Btn onClick={() => { if (!pays || !dob || !source) { setError("Champs requis"); return; } save(2); }} loading={loading}>
  ...
  if (step === 5) return (
    <div style={wrap}>
      ...
      <div style={{ flex: 1, paddingTop: 20, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <FSelect label="Situation professionnelle" value={situationPro} onChange={setSituationPro} options={SITUATIONS_PRO} />
          {employeurLabel && (
          ...
          {error && <p style={{ color: "#FF6B6B", fontSize: "0.85rem" }}>{error}</p>}
          <Btn onClick={() => {
            if (!situationPro || !revenuMensuel || !adresse || !codePostal || !ville) { setError("Veuillez remplir tous les champs obligatoires"); return; }
            save(5);
          }} loading={loading}>
            Continuer <ArrowRight size={18} />
          </Btn>
  ...
  if (step === 6) return (
    <div style={wrap}>
      ...
      <div style={{ flex: 1, paddingTop: 20, display: "flex", flexDirection: "column" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.82rem", marginBottom: 20 }}>Ces informations sont nécessaires à la constitution de votre dossier bancaire.</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div>
            ...
            {ayantsDroit.length === 0 && (
              <div style={{ background: "#2A2A35", borderRadius: 14, padding: "16px", textAlign: "center" }}>
                <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "0.85rem", margin: 0 }}>Aucun ayant droit ajouté</p>
              </div>
            )}

            {ayantsDroit.map((ad, idx) => (
              <div key={idx} style={{ background: "#2A2A35", borderRadius: 14, padding: 16, marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <span style={{ color: "rgba(255,255,255,0.6)", fontSize: "0.82rem", fontWeight: 600 }}>Ayant droit {idx + 1}</span>
                  <button type="button" onClick={() => removeAyantDroit(idx)}
                    style={{ background: "none", border: "none", color: "rgba(255,100,100,0.7)", cursor: "pointer", padding: 4 }}>
                    <Trash2 size={16} />
                  </button>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <FInput label="Prénom" placeholder="Sarah" value={ad.prenom} onChange={(v) => updateAyantDroit(idx, "prenom", v)} />
                    <FInput label="Nom" placeholder="Müller" value={ad.nom} onChange={(v) => updateAyantDroit(idx, "nom", v)} />
                  </div>
                  <FSelect label="Lien de parenté" value={ad.lien} onChange={(v) => updateAyantDroit(idx, "lien", v)} options={LIENS} />
                  <FInput label="Date de naissance" type="date" value={ad.date_naissance} onChange={(v) => updateAyantDroit(idx, "date_naissance", v)} />
                </div>
              </div>
            ))}
          </div>
          ...
          {error && <p style={{ color: "#FF6B6B", fontSize: "0.85rem" }}>{error}</p>}
          <Btn onClick={() => save(6)} loading={loading}>
            Continuer <ArrowRight size={18} />
          </Btn>
        </div>
      </div>
      ...
      <div style={{ flex: 1, paddingTop: 20, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <div style={{ height: 52, background: "#1E1E28", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, color: "rgba(255,255,255,0.4)", fontSize: "1rem", padding: "0 16px", display: "flex", alignItems: "center" }}>
              {email}
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <div style={{ height: 52, minWidth: 76, background: "#2A2A35", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: "0.9rem", fontWeight: 600 }}>
                🇩🇪 +49
              </div>
          ...
          {error && <p style={{ color: "#FF6B6B", fontSize: "0.85rem" }}>{error}</p>}
          <Btn onClick={() => { if (!phone) { setError("Téléphone requis"); return; } save(7); }} loading={loading}>
    ...
    <div style={wrap}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", gap: 20 }}>
        ...
        <Link href="/client/dashboard"
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", maxWidth: 340, height: 54, borderRadius: 999, background: "#005F2D", color: "white", fontWeight: 700, textDecoration: "none" }}>
