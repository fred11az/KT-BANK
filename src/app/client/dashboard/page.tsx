...
export default function ClientDashboard() {
  ...
  function DocsPage() {
    ...
    function openDocBlob(html: string) {
      const pdfScript = `<script>(function(){function ov(){var b=document.getElementById('pdf-btn');if(!b)return;b.onclick=function(){var btn=b;btn.disabled=true;btn.style.opacity='0.7';btn.innerHTML='&#8987;&nbsp;PDF&hellip;';function ls(src,cb,ec){var s=document.createElement('script');s.src=src;s.onload=cb;s.onerror=ec;document.head.appendChild(s);}function fp(){btn.disabled=false;btn.style.opacity='1';btn.innerHTML='&#8595;&nbsp;PDF';window.print();}function go(){var pp=Array.from(document.querySelectorAll('.page-bg .page'));if(!pp.length||!window.jspdf||!window.html2canvas){fp();return;}var pdf=new window.jspdf.jsPDF('p','mm','a4'),i=0;(function nx(){if(i>=pp.length){pdf.save('KT-Bank-Document.pdf');btn.disabled=false;btn.style.opacity='1';btn.innerHTML='&#8595;&nbsp;PDF';return;}window.html2canvas(pp[i],{scale:2,useCORS:true,logging:false}).then(function(c){var h=Math.round(c.height*210/c.width);if(i>0)pdf.addPage();pdf.addImage(c.toDataURL('image/jpeg',0.95),'JPEG',0,0,210,h);i++;nx();}).catch(fp);})();}if(window.html2canvas&&window.jspdf){go();}else{ls('https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js',function(){ls('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js',go,fp);},fp);}};}if(document.readyState==='complete'){setTimeout(ov,300);}else{window.addEventListener('load',function(){setTimeout(ov,300);});}})();<\/script>`;
